...
const openai_1 = require("../../src/infrastructure/ai/openai");
describe('AI Services', () => {
    describe('Burnout Analysis', () => {
        it('should analyze burnout risk from session data', async () => {
            ...
            const result = await openai_1.openaiService.analyzeBurnoutRisk('user-123', sessionData, 'org-123');
        ...
        it('should provide fallback burnout analysis without OpenAI', async () => {
            ...
            const result = await openai_1.openaiService.analyzeBurnoutRisk('user-456', sessionData, 'org-123');
    ...
    describe('Coaching Generation', () => {
        it('should generate personalized coaching recommendations', async () => {
            ...
            const result = await openai_1.openaiService.generateCoaching('user-123', userData, 'org-123');
        ...
        it('should timeout coaching generation after AI_TIMEOUT_MS', async () => {
            ...
            const result = await openai_1.openaiService.generateCoaching('user-789', userData, 'org-123');
            expect(result).toBeDefined();
        });
    });
    describe('Attendance Analysis', () => {
        it('should analyze attendance patterns and anomalies', async () => {
            ...
            const result = await openai_1.openaiService.analyzeAttendance('user-123', attendanceData, 'org-123');
            expect(result).toHaveProperty('success', true);
            expect(result).toHaveProperty('data');
    ...
    describe('Project Risk Analysis', () => {
        it('should identify project risks and mitigation', async () => {
            ...
            const result = await openai_1.openaiService.analyzeProjectRisk('proj-123', projectData, 'org-123');
            expect(result).toHaveProperty('success', true);
            expect(result).toHaveProperty('data');
    ...
    describe('AI Service Caching', () => {
        it('should cache AI results for performance', async () => {
            ...
            // First call
            const result1 = await openai_1.openaiService.generateCoaching('user-cache-test', userData, 'org-123');
            // Second call (should use cache)
            const result2 = await openai_1.openaiService.generateCoaching('user-cache-test', userData, 'org-123');
    ...
    describe('AI Service Rate Limiting', () => {
        it('should enforce rate limits per tenant', async () => {
            ...
            const requests = Array(5)
                .fill(userData)
                .map((data, i) => openai_1.openaiService.generateCoaching(`user-${i}`, data, 'org-rate-limit-test'));
